...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
.. _figure_visr_tutorial_visr_composite_component_example_vbap_panner:

.. figure:: images/vbap_panner.png
   :width: 80 %
   :align: center   
...
We demonstrate that by building a simple multichannel amplitude panning renderer, which is schematically depicted in Figure :ref:`figure_visr_tutorial_visr_composite_component_example_vbap_panner`.
...
This starts by importing several Python modules that provide VISR functionality, namely :code:`visr` for the core API, :code:`rcl` for the built-in component library (see Sec. :ref:`visr_tutorial_subsec_visr_framework_rcl`) and the :code:`pml` for parameter data and communication protocols (Sec. :code:`visr_tutorial_subsec_visr_framework_ports`).
...
As explained in Sec. :ref:`visr_tutorial_subsec_visr_framework_composite_components`, the functionality of a composite component is embodied in its constructor, which defines external inputs and outputs, the contained components, and their connections.
...
In the VISR framework, this is best done by creating a new top-level composite component, denoted :code:`RealtimeVbapRenderer` in Fig. :ref:`figure_visr_tutorial_visr_composite_component_example_vbap_panner`, that contains the :code:`VbapRenderer` and means to receive and decode object metadata from network messages.
...
.. _figure_visr_tutorial_example_amplitude_panning_auralization:

.. figure:: images/vbap_panner_auralisation.png
   :width: 80 %
   :align: center   
...
To this end, we create a new top-level component depicted in Figure :code:`figure_visr_tutorial_example_amplitude_panning_auralization`.
...
The novelty of this approach is that it uses a convex optimization algorithm to compute a solution with similar properties to VBAP, but uses :math:`\ell_{2}` optimality to resolve ambiguity issues of VBAP, see :cite:`hughes_et_al2018_dual_frequency_amplitude_panning_for_multichannel_audio_systems`.
Using the VISR framework it is possible to use the rich set of libraries for technical and scientific computing available in Python, in this case the numerical optimization package **cvxpy** :cite:`diamond_boyd2016_cvxpy`, for algorithm prototyping and realtime evaluation.
...
The :code:`process()` function accesses the sample data of the audio ports as :code:`NumPy` arrays and uses the matrix multiplication operator :code:`@` to calculate the result.
...
.. _figure_visr_tutorial_example_offline_simulation_audio_signal:

.. figure:: images/offline_simulation_audio_signal.png
   :width: 80 %
   :align: center   
...
Figure :ref:`figure_visr_tutorial_example_offline_simulation_audio_signal` shows the output signal for one loudspeaker (top-rear right position :code:`U-135`)) of a 9-loudspeaker setup according to ITU-R BS.2051 :cite:`itu2017-itu-r-bs2051-1` and compares it to the output of a standard VBAP implementation.
The sound object is a sinusoidal tone rotating around the central listener at a constant elevation of 10 degree.
...
.. _figure_visr_tutorial_example_offline_simulation_gains:

.. figure:: images/offline_simulation_gains.png
   :width: 80 %
   :align: center   
...
Figure :ref:`figure_visr_tutorial_example_offline_simulation_gains` shows the resulting gains of the :code:`VbapL2Panner` and the standard VBAP algorithm for the upper-rear loudspeakers :code:`U-135` and :code:`U+135` as a function of the source's azimuth.
...
This is in accordance with the audio signal depicted in Figure :ref:`figure_visr_tutorial_example_offline_simulation_audio_signal`, and is caused by ambiguities inherent to the VBAP algorithm, see e.g., :cite:`franck_wang_fazi2017_sparse_l1-optimal_multiloudspeaker_panning_and_its_equivalence_to_vector_based_amplitude_panning`.
...
.. _figure_visr_tutorial_example_offline_simulation_energy_direction_difference:

.. figure:: images/offline_simulation_energy_direction_difference.png
   :width: 80 %
   :align: center   
   
   Offline simulation plots of the proposed panning algorithm: Energy vector difference.


Access to such data also enables more sophisticated analyses.
As an example, :ref :`figure_visr_tutorial_example_offline_simulation_energy_direction_difference` shows the difference of the energy vector, a metric to estimate the localisation of mid-to-high frequency content, for both algorithms. It shows that the error is smaller and more even for the proposed VBAP L2 algorithm than for the standard algorithm. 
...
.. _figure_visr_tutorial_example_panning_comparison_signal_flow:

.. figure:: images/panning_comparison.png
   :width: 80 %
   :align: center   

   Subjective listening test application: Signal flow.
..
   \begin{figure}
     \centering
...
With the VISR framework, the signal processing is implemented as a new top-level composite component, shown in Figure :ref:`figure_visr_tutorial_example_panning_comparison_signal_flow`, which contains the proposed renderer as one component, alongside renderers for other rendering schemes.
...
.. _figure_visr_tutorial_example_panning_comparison_ui:

.. figure:: images/listening_test_gui.png
   :width: 80 %
   :align: center   
...
.. _figure_visr_tutorial_example_max_msp:

.. figure:: images/max_msp_example.png
   :width: 80 %
   :align: center   

   Interactive loudspeaker rendering binauralization implemented in
   Max/MSP using VISR components as externals.
...
Figure :ref:`figure_visr_tutorial_example_max_msp` shows an example of an interactive multichannel loudspeaker rendering combined with a binaural auralization, both implemented as Max/MSP externals.
...
.. _figure_visr_tutorial_example_object_renderer_vst:

.. figure:: images/object_renderer_plugin.png
   :width: 80 %
   :align: center   
...
The second example, depicted in Fig. :code:`igure_visr_tutorial_example_object_renderer_vst`, shows a DAW plugin that renders a point source object to a multi-loudspeaker setup.
